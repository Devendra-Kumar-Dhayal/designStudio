  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (element.type !== "line") {
      ...
      if (!seen[element.options.meta.common.label]) {
        ...
        seen[element.options.meta.common.label] = len;
      }
      else {
        const index = seen[element.options.meta.common.label] - 1;
        const elementToPush = {
          ...uniqueArray[index],
          diff: [
            ...(uniqueArray[index]?.diff ?? []),
            { ...element.currentWorkspace, meta: element.options },
          ],
        };
        uniqueArray[index] = elementToPush;
      }
    }
  }
